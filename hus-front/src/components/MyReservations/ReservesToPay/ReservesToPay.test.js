...
import ReservesToPay from "./ReservesToPay";
afterEach(cleanup);
const product = {
  id: 1,
...
it("Render Paid Reservations", () => {
  ...
  const  ReservesToPayContainer = render(
    <MemoryRouter initialEntries={[`/my-reservations`]}>
      <ReservesToPay data={[product]}/>
    </MemoryRouter>
  );
  expect (ReservesToPayContainer.container).toHaveTextContent("Reservas por pagar");
});