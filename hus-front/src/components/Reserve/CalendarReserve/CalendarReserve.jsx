...
import { CalendarReserveContainer } from "./CalendarReserveStyled";
...
export default function CalendarReserve({
  ...
  useEffect(() => {
    ...
    if (dates !== undefined) {
      ...
      setDisabledDates(array.sort((a, b) => a - b));
    }
  }, [dates]);

  const onChange = (date) => {
    setDate(date);
    setCheckIn(date[0].toISOString().slice(0, 10));
    setCheckOut(
      date[1].getFullYear() +
        "-" + 
