...
it("Render ReservationForm", () => {
  ...
  useState.mockImplementation(() => [false, setRequest]);
  const setDisabledDates = jest.fn();
  useState.mockImplementation(() => [[
    new Date(),
    new Date()], setDisabledDates]);
  ...
  const reservationForm = render(
    <MemoryRouter initialEntries={[`/reservation/product/${product.id}`]}>
